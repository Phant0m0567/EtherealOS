...
//#region windows/node_modules/i18next-xhr-backend/dist/esm/i18nextXHRBackend.js
var arr = [];
...
function addQueryString(url, params) {
	if (params && _typeof(params) === "object") {
		...
		for (var paramName in params) queryString += "&" + e(paramName) + "=" + e(params[paramName]);
...
function ajax(url, options, callback, data, cache) {
	if (data && _typeof(data) === "object") {
		if (!cache) data["_t"] = /* @__PURE__ */ new Date();
		data = addQueryString("", data).slice(1);
	}
	if (options.queryStringParams) url = addQueryString(url, options.queryStringParams);
	try {
		var x;
		if (XMLHttpRequest) x = new XMLHttpRequest();
		else x = new ActiveXObject("MSXML2.XMLHTTP.3.0");
	...
	_createClass(Backend, [
		...
		},
		{
			...
			value: function read(language, namespace, callback) {
				var loadPath = this.options.loadPath;
				if (typeof this.options.loadPath === "function") loadPath = this.options.loadPath([language], [namespace]);
				var url = this.services.interpolator.interpolate(loadPath, {
					lng: language,
					ns: namespace
				});
