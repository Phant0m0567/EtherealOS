...
export const StartMenu = () => {
  ...
  const clickDispatch = (event) => {
    ...
    if (action.type == "PINSTART") {
      Actions.pinAppToStart(action.payload);
      dispatch({ type: "STARTHID" });
      return;
    }

    if (action.type == "PINTASKBAR") {
      Actions.pinAppToTaskbar(action.payload);
      dispatch({ type: "STARTHID" });
      return;
    }

    if (action.type == "UNINSTALLAPP") {
      Actions.uninstallApp(action.payload);
      dispatch({ type: "STARTHID" });
      return;
    }
          ...
          <div className="shResult w-full flex justify-between">
              ...
              {query.length ? (
                ...
                <>
                  <div className="topApps flex w-full justify-between">
                    {start.rcApps.slice(0, 4).map((app, i) => {
                      return (
                        <div
                          key={i}
            ...
            {query.length ? (
              <div className="w-2/3 rightSide rounded">
                ...
                >
                  <Icon className="blueicon" src="pin" width={16} />
