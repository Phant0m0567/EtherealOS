...
export const Spotify = () => {
  ...
  const action = (e) => {
    ...
    if (act == "discv") {
      ...
      setProg(0);
      setPerProg(0);
      var ip = (curr + 1) % Math.max(queue.length || 1, 1);
      if (shfle) ip = mxqueue[curr] ?? 0;
      setCurr(ip);
    } else if (act == "prev") {
      setPause(false);
      setProg(0);
      setPerProg(0);
      var ip = (curr - 1 + (queue.length || 1)) % (queue.length || 1);
      if (shfle) ip = mxqueue[curr] ?? 0;
      setCurr(ip);
    } else if (act == "album") {
      if (payload.includes(",")) {
      ...
      if (songid != (queue[curr] && queue[curr].id)) {
        jiosaavn
          .fetchSong(songid)
          .then((res) => {
  ...
  const handleChange = (e) => {
    ...
    if (!audiosrc || !currentTrack) return;
    var ip = e.target.value;
    audiosrc.currentTime = ip;
    setPerProg(ip / (currentTrack.duration || 1));
    setProg(floor(ip));
  };
  ...
  useEffect(() => {
    const safeQueue = Array.isArray(queue) && queue.length ? queue : [{ }];
    const safeCurr = Number.isInteger(curr) && curr >= 0 && curr < safeQueue.length ? curr : 0;

    if (!safeQueue[safeCurr] || safeQueue[safeCurr].name == null) {
      jiosaavn
        .getDefault()
  ...
  const safeQueue = Array.isArray(queue) && queue.length ? queue : [{}];
  const safeCurr = Number.isInteger(curr) && curr >= 0 && curr < safeQueue.length ? curr : 0;
  const currentTrack = safeQueue[safeCurr] || {};
            ...
            <div className="splayer">
              <div className="snfo flex items-center">
                {currentTrack.albumArt ? (
                  <Image src={currentTrack.albumArt.to150()} w={56} ext lazy />
                ) : (
                ...
                <div className="sname ml-3">
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: currentTrack.name || "Album",
                    }}
                  ></div>
                  <div
                    className="text-xs tracking-wider text-gray-400"
                    dangerouslySetInnerHTML={{
                      __html: currentTrack.artist || "Artist",
                    }}
                  ></div>
                </div>
              </div>
              <div className="songAct" data-prtclk={!currentTrack.name}>
                ...
                <div className="w-full flex items-center mt-2 justify-center">
                  ...
                  <div className="progTime">
                    {jiosaavn.formatTime(currentTrack.duration || 0)}
                  </div>
                </div>
              </div>
...
const Playlist = ({ type, tdata, action, action2, sid, paused }) => {
  ...
  useEffect(() => {
    ...
    if (type == "album") {
      ...
      setData(tdata || {});
      var tmptot = 0;
      var songs = (tdata && tdata.songs) || [];
      for (var i = 0; i < songs.length; i++) {
        tmptot += parseInt(songs[i].song_duration);
      }
      setTotime(tmptot);
    }
  }, [tdata]);
  ...
  return (
    <div className="mt-12">
      ...
      <div className="songCont prtclk acol py-2 pr-12">
        ...
        <Image
          src={(currentTrack.albumArt && currentTrack.albumArt.to150()) || "/img/asset/mixdef.jpg"}
          w={40}
          ext
          lazy
        />
        <div className="flex flex-col">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: currentTrack.artist || "Unknown artist" }}
          ></div>
        </div>
        <div
          className="text-sm dotdot font-semibold"
          dangerouslySetInnerHTML={{ __html: currentTrack.album || "Unknown album" }}
        ></div>
        <div className="text-sm font-semibold">
          {jiosaavn.formatTime(currentTrack.duration || 0)}
        </div>
      ...
      {jiosaavn.sliceArr(queue, curr).map((qs, i) => {
        return (
          ...
          >
            ...
            <Image
              src={(qs.albumArt && qs.albumArt.to150()) || "/img/asset/mixdef.jpg"}
              w={40}
              ext
              lazy
            />
