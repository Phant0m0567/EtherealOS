...
export const pinAppToStart = (name) => {
  if (!name) return;
  ...
  if (app) {
    store.dispatch({ type: "STARTPIN", payload: app });
  }
};

export const pinAppToTaskbar = (name) => {
  if (!name) return;

  var taskbar = localStorage.getItem("taskbar");
  if (!taskbar) taskbar = "[]";
  taskbar = JSON.parse(taskbar);

  if (!taskbar.includes(name)) {
    taskbar.push(name);
  ...
  if (app) {
    store.dispatch({ type: "TASKPIN", payload: app });
  }
};

export const uninstallApp = (name) => {
  if (!name) return;
  ...
  var installed = localStorage.getItem("installed");
  if (!installed) installed = "[]";
  installed = JSON.parse(installed).filter(
    (item) => item.icon != app.icon && item.name != app.name,
  );
  localStorage.setItem("installed", JSON.stringify(installed));

  ["desktop", "pinned", "taskbar"].forEach((listName) => {
    var list = localStorage.getItem(listName);
    if (!list) return;

    list = JSON.parse(list).filter((item) => item != app.name);
    localStorage.setItem(listName, JSON.stringify(list));
  });

  store.dispatch({ type: "DELAPP", payload: app.icon });
  store.dispatch({ type: "DESKREM", payload: app.name });
  store.dispatch({ type: "TASKREM", payload: app.name });
  store.dispatch({ type: "STARTREM", payload: app.name });
};
